...
if len(sys.argv) not in (5, 6):
    print("Usage: delete_oidc_provider_role.py <role_name> <policy_name> <s3_compatible_endpoint> <iam_client_id> <iam_client_password> [region='']")
    sys.exit(1)
...
print("delete_role_policy() Policy name '"+role_name+"'.")
try:
    role_response = iam_client.delete_role_policy(
        RoleName=role_name,
        PolicyName=policy_name
    )
    print("Successfully Deleted Policy "+role_name+" for role "+policy_name+".")
