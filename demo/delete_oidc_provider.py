...
if len(sys.argv) not in (5, 6):
    print("Usage: delete_oidc_provider.py <oidc_arn> <s3_compatible_endpoint> <iam_client_id> <iam_client_password> [region='']")
    sys.exit(1)
...
region = sys.argv[5] if len(sys.argv) == 6 else ''
...
try:
    print("Deleting OIDC provider arn: '"+oidc_arn+"'.")
